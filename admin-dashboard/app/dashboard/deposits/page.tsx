          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 text-left">
              <tr>
                ...
                <th className="px-3 sm:px-4 py-2 sm:py-3">Employee</th>
                <th className="px-3 sm:px-4 py-2 sm:py-3">Amount</th>
                <th className="px-3 sm:px-4 py-2 sm:py-3">Bank</th>
            ...
            <tbody className="divide-y">
              {deposits.map((d) => (
                <tr key={d.id} className="hover:bg-gray-50">
                  <td className="px-3 sm:px-4 py-2 sm:py-3 font-mono text-xs">{d.referenceNumber}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 font-medium">{d.deliveryEmployeeCode}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3">R {d.amount.toFixed(2)}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3">{d.bankName ?? '—'} <span className="text-xs text-gray-400">{d.bankReference}</span></td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${statusColors[d.status]}`}>
                      {statusLabels[d.status]}
                    </span>
                  </td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3 text-gray-500">{new Date(d.submittedAt).toLocaleDateString()}</td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3">
                    {d.status === 0 && (
